...
class GoProGPSApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("GoPro Street View Helper (macOS)")
    ...
    def process_files(self):
        ...
        try:
            # 1. 撮影日時抽出
            res = subprocess.run([exiftool_cmd, "-CreateDate", "-s3", "-S", self.path_360], 
            ...
            # 3. GPX作成
            cmd = [exiftool_cmd, "-api", "TimePrecision=3", "-p", gpx_fmt, "-ee", "-c", "%.8f", self.path_360]
